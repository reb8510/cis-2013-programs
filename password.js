...
var passwordCheckOld = function () 
{
   ...
   */
  if (stringPassword.length < 8) //check to see if pass is less than 8 characters long
  {
      stringOutput = ("Your password needs to be at least 8 characters long");
  }
  else
  {
   for (var i = 0; i < arrayBadPasswords.length; i++) //process through an array, checks to see if the pass is on the arrayBadPasswords list
   {
      if (stringPassword === arrayBadPasswords[i]) // checks to see if a variable matches a value in the arrayBadPasswords
      {
         stringOutput = ("The password matches a bad password on the list."); //if the pass matches a bad password on the list, this output appears
         
      }
   }
  }
  if (stringOutput == "") //if the output is not on the arrayBadPasswords list it will fall into this if statement
{
   stringOutput = "The output password is good.";
  }
